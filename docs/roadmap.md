...
| Manifesto de cores fica obsoleto sozinho (hash mismatch recorrente) | P | **Achado em 2026-09-06:** `internal/install/data/retroarch_cores_manifest.json` guarda um SHA256 medido num instante contra uma URL `.../nightly/<plataforma>/latest/<arquivo>` — um alvo que o buildbot.libretro.com reconstrói periodicamente. Toda vez que isso acontece, o hash salvo fica velho e `StartCore` recusa (corretamente) instalar o core novo por não bater — foi o que aconteceu com o `mgba` em Windows nesse dia (hash esperado `d41088cd...`, recebido `77edb521...`; resolvido rodando `./cmd/generate-retroarch-manifest` de novo). Não é bug: é a checagem de integridade funcionando. **O que falta:** ou aceitar que isso volta a acontecer e regenerar manualmente quando reportado (caminho atual), ou o ZeuX buildar contra uma versão pinada/datada do core em vez de "latest" (mudaria a URL do manifesto e o próprio gerador) — decisão de escopo que ainda não foi tomada |
